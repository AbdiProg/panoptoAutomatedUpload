...
def mergeIntoPlaylist(sessionArray, playlistName, folderID):
    ...
    response = requests_session.post('https://test-tu-darmstadt.cloud.panopto.eu/Panopto/api/v1/playlists',
                                     json=payload)


# Die directoryPanoptoID ist konkret eine OrdnerID für einen Fachbereich
def uploadVideos(sourcePath, directoryPanoptoID):
    for playlistname in os.listdir(
            sourcePath):  # Hier iteriert man durch die zukünftigen Playlists, die noch als Ordner vorhanden sind.
        ...
        if os.path.isdir(p):
            for videoname in os.listdir(p):
                ...
                uploader = PanoptoUploader('test-tu-darmstadt.cloud.panopto.eu', True, oauth2, videoname,
                                           "Ein TestVideo")
                uploader.upload_video(v, playlistDirectoryID)
            respVideos = requests_session.get(
                "https://test-tu-darmstadt.cloud.panopto.eu/Panopto/api/v1/folders/{0}/sessions".format(
                    playlistDirectoryID))
